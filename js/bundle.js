...
function forms(formSelector, modalTimerId) {
  ...
  function bindPostData(form) {
    form.addEventListener("submit", (e) => {
      ...
      // down - fetch
      //   take argument from json server url in terminal
...
function slider({container, slide, nextArrow, prevArrow, totalCounter, currentCounter, wrapper, field}) {
  ...
  next.addEventListener('click', () => {
      ...
      const slideWidth = parseInt(width, 10);
  ...
  dots.forEach(dot => {
      dot.addEventListener('click', (e) => {
          const slideTo = +e.target.getAttribute('data-slide-to'); // Перетворення на число
          console.log('Slide To:', slideTo);

          slideIndex = slideTo;
          const slideWidth = +width.slice(0, width.length - 2); // Перетворення на число
          console.log('Slide Width:', slideWidth);
...
window.addEventListener("DOMContentLoaded", function () {
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_7__["default"])('.timer', '2024-11-10');
